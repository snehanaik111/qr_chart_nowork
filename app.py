...
def add_sensor_data(adminname):
    try:
        ...
        # Reflect the table to use it dynamically
        metadata = MetaData()
        ...
        sensor_data = parsed_data.get('data')[0]  # Assuming sensor_data is an array
        vehicleno = parsed_data.get('Vehicle no')
        volume_liters = get_volume(sensor_data)

        # Generate QR Code and PDF with full row data
...
def generate_qr_code(data):
    """Generate a QR code with full row data."""
    qr = qrcode.QRCode(
        version=1,
        error_correction=qrcode.constants.ERROR_CORRECT_L,
        box_size=10,
    ...
    qr.add_data(json.dumps(data))  # Add full row data to the QR code
    qr.make(fit=True)
    img = qr.make_image(fill='black', back_color='white')
    buf = BytesIO()
    img.save(buf, format='PNG')
...
def generate_pdf(data):
    """Generate a PDF with full row data."""
    buffer = BytesIO()
    c = canvas.Canvas(buffer, pagesize=letter)
    c.setFont("Helvetica-Bold", 14)
    c.drawString(100, 780, "Sensor Data Report")
    c.line(50, 770, 550, 770)
    c.setFont("Helvetica", 10)
    c.drawString(100, 750, f"Date: {data['date']}")
    c.drawString(100, 730, f"Full Address: {data['full_addr']}")
    c.drawString(100, 710, f"Sensor Data: {data['sensor_data']}")
    c.drawString(100, 690, f"Vehicle No: {data['vehicleno']}")
    c.drawString(100, 670, f"Volume (liters): {data['volume_liters']}")
    c.line(50, 660, 550, 660)
    c.showPage()
    c.save()
    buffer.seek(0)
